...
def precompute_rope_cache(dim: int, seq_len: int, theta: float = 10000.0, device=None) -> torch.Tensor:
    """
    ...
    Returns:
        Cached sin/cos tensors for RoPE
    """
    ...
    # Create cache of sines and cosines
...
def apply_rope(x: torch.Tensor, rope_cache: torch.Tensor) -> torch.Tensor:
    ...
    # Apply rotation using the rotary formulation
    x_rot_new = x_rot * cos - torch.cat([-x_pass[..., -1:], x_pass[..., :-1]], dim=-1) * sin
    x_pass_new = x_pass * cos + torch.cat([x_rot[..., -1:], x_rot[..., :-1]], dim=-1) * sin
    
    # Interleave the rotated and passed values
    return torch.flatten(torch.stack([x_rot_new, x_pass_new], dim=-1), start_dim=-2)
...
class GroupQueryAttention(nn.Module):
    def __init__(self, config):
        ...
        print(f"Initializing GQA with: n_embd={self.n_embd}, n_head={self.n_head}, "
              f"n_query_groups={self.n_query_groups}, head_size={self.head_size}, "
              f"heads_per_group={self.heads_per_group}")

        # Single projection for queries (full size)
        self.q_proj = nn.Linear(config.n_embd, config.n_embd, bias=config.bias)
        
        # Reduced projections for keys and values
        kv_dim = self.head_size * self.n_query_groups
        ...
        if not self.flash:
            ...
            self.register_buffer("mask", causal_mask.view(1, 1, config.block_size, config.block_size))
    ...
    def forward(self, x):
        ...
        k = self.k_proj(x)  # (B, T, head_size * n_query_groups)
        v = self.v_proj(x)  # (B, T, head_size * n_query_groups)
        
        # Print debug info for first forward pass
        if not hasattr(self, '_printed_debug'):
            print(f"\nDebug shapes:")
            print(f"Input x: {x.shape}")
            print(f"After q_proj: {q.shape}")
            print(f"After k_proj: {k.shape}")
        ...
        k = k.transpose(1, 2)  # (B, n_query_groups, T, head_size)
        v = v.transpose(1, 2)  # (B, n_query_groups, T, head_size)
        
        # Expand k and v for all heads
        k = k.repeat_interleave(self.heads_per_group, dim=1)  # (B, n_head, T, head_size)
        v = v.repeat_interleave(self.heads_per_group, dim=1)  # (B, n_head, T, head_size)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # Key, query, value projections in a single matrix
        ...
        if not self.flash:
            ...
            # Create causal mask buffer for manual attention
            mask = torch.tril(torch.ones(config.block_size, config.block_size))
            mask = mask.view(1, 1, config.block_size, config.block_size)
            self.register_buffer("mask", mask)
        
        # Initialize RoPE cache as None - will be created on first forward pass
        self.rope_cache = None
        
    def forward(self, x):
        B, T, C = x.size()  # batch size, sequence length, embedding dim
        
        # Create RoPE cache if needed
        if self.rope_cache is None or self.rope_cache.shape[0] < T:
            self.rope_cache = precompute_rope_cache(
                self.head_size,
                max(T, 32),  # minimum size of 32 for efficiency
                device=x.device
            )
        ...
        # Compute attention using either flash attention or manual implementation
        if self.flash:
...
class Block(nn.Module):
    """
    Transformer block with GroupQueryAttention and optional parallel computation
    """
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, x):
        if self.parallel_residual:
            ...
            norm_x = self.ln_1(x)
            return x + self.attn(norm_x) + self.mlp(norm_x)
        else:
