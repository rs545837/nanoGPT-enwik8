...
class Block(nn.Module):
    def __init__(self, config):
        ...
        self.layer_idx = None  # Will be set during model initialization
        ...
        # Auxiliary prediction heads for intermediate layers - only if specified
        if getattr(config, 'use_auxiliary_loss', False):
            self.aux_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
        
        self.resid_scale = 1 / math.sqrt(2 * config.n_layer)
    
    def forward(self, x, return_aux_logits=False):
        if self.use_parallel_residual:
            normed_x = self.ln_1(x)
            attn_output = self.attn(normed_x)
            mlp_output = self.mlp(self.ln_2(normed_x))
            
            x = x + self.resid_scale * (attn_output + mlp_output)
        else:
            attn_output = self.attn(self.ln_1(x))
            x = x + self.resid_scale * attn_output
            
            mlp_output = self.mlp(self.ln_2(x))
            x = x + self.resid_scale * mlp_output
            
        # Return auxiliary logits only if head exists and requested
        if return_aux_logits and hasattr(self, 'aux_head'):
...
class GPTConfig:
    ...
    use_parallel_residual: bool = False  # Compute attention and MLP in parallel
    
    # Auxiliary loss settings
    use_auxiliary_loss: bool = True
    aux_loss_layers: str = 'all'  # 'all' or list of layer indices
    aux_loss_weight: float = 0.5  # Weight for auxiliary losses
    multiple_targets: int = 2     # Number of future tokens to predict
    ...
    use_checkpointing: bool = True   # Enable gradient checkpointing for memory efficiency
...
class GPT(nn.Module):
    ...
    def load_state_dict(self, state_dict, strict=True):
        """Override load_state_dict to handle auxiliary heads"""
        if not self.config.use_auxiliary_loss:
            # Filter out auxiliary head keys if we're not using them
            state_dict = {k: v for k, v in state_dict.items() 
                         if not k.endswith('.aux_head.weight')}
    ...
    def forward(self, idx, targets=None):
        ...
        # Through the transformer blocks with auxiliary losses
        for i, block in enumerate(self.transformer.h):
            if getattr(self.config, 'use_auxiliary_loss', False) and (
                self.config.aux_loss_layers == 'all' or 
                i in self.config.aux_loss_layers
            ):
                x, aux_logits = block(x, return_aux_logits=True)
                
                # Calculate auxiliary loss for this layer
                if targets is not None:
                    ...
                    # Scale loss based on layer depth
                    layer_weight = (i + 1) / len(self.transformer.h)
                    aux_losses.append(aux_loss * layer_weight)
                    
                layer_outputs.append(aux_logits)
            else:
                x = block(x)
        ...
        if targets is not None:
            ...
            # Combine with auxiliary losses if they exist
            if aux_losses and getattr(self.config, 'use_auxiliary_loss', False):
                aux_loss = sum(aux_losses) / len(aux_losses)
                aux_weight = getattr(self.config, 'aux_loss_weight', 0.5)
